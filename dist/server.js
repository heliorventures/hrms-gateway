...
import { SUBGRAPHS, subgraphUrl } from "./subgraphs.js";
 ...
 */
function forwardHeaders(executorRequest) {
    const ctx = executorRequest?.context;
    const headers = {};
    if (ctx?.tenantId)
        headers[TENANT_HEADER] = ctx.tenantId;
    if (ctx?.authorization)
        headers[AUTH_HEADER] = ctx.authorization;
    return headers;
}
async function loadSubgraph(def) {
    ...
    try {
        ...
        console.warn(`[gateway] \u26A0 ${def.name.padEnd(14)} unreachable at ${url} \u2014 skipping. (${err.message})`);
        return null;
    }
}
async function main() {
    console.log("[gateway] booting KabiPay stitching gateway\u2026");
    const loaded = (await Promise.all(SUBGRAPHS.map((def) => loadSubgraph(def)))).filter((s) => s !== null);
    if (loaded.length === 0) {
        console.error("[gateway] no subgraphs reachable \u2014 serving empty schema; start subgraphs and restart the gateway.");
    }
    ...
    server.listen(GATEWAY_PORT, () => {
        console.log(`[gateway] listening on http://127.0.0.1:${GATEWAY_PORT}/graphql with ${loaded.length}/${SUBGRAPHS.length} subgraphs`);
    });
}
main().catch((err) => {
